...
"if exists("loaded_alternateFile1")
"    finish
"endif
...
function! <SID>GetSingleParamInfo(s, isparam)
	...
	" insert black to * (&), eg int *i => int * i
...
function! <SID>GetDoxygenStyleDoc(fun_info, leading_blank)
	...
	let doc .= a:leading_blank." * @brief brief description about ".fun_name."\n"
	let doc .= a:leading_blank." *\n"
	let doc .= a:leading_blank." * detail description about ".fun_name."\n"
	...
	while(strlen(type)>0 && strlen(name)>0)
		...
		let doc .= a:leading_blank." * @param ".name." "."a ".type.  "\n"
		let type = <SID>GetNthItemFromList(a:fun_info, idx, ",")
		let idx += 1
		let name = <SID>GetNthItemFromList(a:fun_info, idx, ",")
		let idx += 1
	...
	if (strlen(ret_type)>0) 
		let doc .= a:leading_blank." * @return a ".ret_type. " \n"
	endif

	"other
	let doc .= a:leading_blank." * @author ".<SID>GetUserName()."\n" 
	let doc .= a:leading_blank." * @date ".<SID>GetDate()."\n" 
	let doc .= a:leading_blank." * @version 1.0.0 \n" 
	let doc .= a:leading_blank." * @todo \n" 
...
map <F12> :call <SID>DateInsert()<CR>
map <F11> :call <SID>OpenNew()<CR>
map <F10> :call <SID>GenDoc()<CR>
