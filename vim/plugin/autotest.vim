...
"打印函数对应的case数量的统计信息
function! <SID>Print_statics( class_name, fun_name )
	...
	if a:class_name != ""
		if g:class_to_class_flag == 0
			call system('echo '.prefix ."line:".line_num . " class:". a:class_name. " function:".a:fun_name ." case:". case_count .'>> .statics_temp')
		else
			call system('echo '.prefix ."line:".line_num . " class:". a:class_name. " function:".a:fun_name ." case:". case_count .'>> .statics_temp')
		endif
	else 
		call system('echo '.prefix . "line:".line_num ." function:".a:fun_name ." case:". case_count.'>> .statics_temp')
	endif
	w
endfunctio

"追加统计信息
function! <SID>Print_append( class_name, fun_name )
	let prefix = ""
	let line_num = line(".")
	exec 'edit .statics_temp'
	if a:class_name != ""
		call append(line("$") - 1,prefix ."line:".line_num . " class:". a:class_name. " function:".a:fun_name )
	else 
		call append(line("$") - 1,prefix . "line:".line_num ." function:".a:fun_name )
	endif
	w
	exec 'edit '.g:source_path
endfunctio
	...
	while 1
		...
		if fun_name !=""
			...
			if g:flag_c_cpp == 1 && class_name != "" 
				...
				"echo "0==================".class_name.":".fun_name.":".<SID>Is_class_fun().":".g:return_type."\n"
                "echo g:class_or_struct."********************".class_name
				"echo "fun_name :".fun_name
				if class_name == "" && fun_name == 'main'
					let xxx="do nothing"
				elseif  g:return_type == ""
					if class_name != "" && class_name != '#is_private_fun' && <SID>Is_class_fun() == 1

						if create_flag == 0
							call <SID>Create_file()
							let create_flag = 1
						endif
						"echo "1==================".class_name.":".fun_name."\n"
						call <SID>Create_test(class_name, fun_name)
...
function! <SID>List_statics()
	...
	while 1
		...
		if fun_name !=""
			let class_name = <SID>Get_colon_classname(fun_name)
			if g:flag_c_cpp == 1 && class_name != "" 
				call <SID>Print_statics(class_name, fun_name)
			else
				let class_name = <SID>Get_class_name()
				if class_name == "" && fun_name == 'main'
					let xxx="do nothing"
				elseif  g:return_type == ""
					if class_name != "" && class_name != '#is_private_fun' && <SID>Is_class_fun() == 1
						call <SID>Print_statics(class_name, fun_name)
					elseif class_name == "" && <SID>Not_in_codeblock('{','','}') == 1
						call <SID>Print_statics("", fun_name)
					endif
				elseif class_name != "" && class_name != '#is_private_fun' && <SID>Is_class_fun() == 1
					call <SID>Print_statics(class_name, fun_name)
				elseif class_name == "" && g:return_type != 'throw' && g:return_type != 'new'
	...
	while 1
		...
		if fun_name !=""
			...
			if g:flag_c_cpp == 1 && class_name != "" 
				...
				if temp == 0
					call <SID>Print_append(class_name, fun_name)
					call <SID>Create_test(class_name, fun_name)
				endif
